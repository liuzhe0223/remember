...
func DoOp(robj *dt.Robj, method, op string, params map[string]string) (resValue []dt.Robj) {

	realOp, in := getRealOpAndParams(methodMap[method][op], params)

	var resReflectValues []reflect.Value
	//todo: ad more op
	switch robj.Type {
	case dt.RlistType:
		resReflectValues = reflect.ValueOf(robj.Obj.(dt.Rlist)).MethodByName(realOp).CallSlice(in)
	default:
